 ...
 * @returns {function(*)}
 */
export let getOrderList = (array, userId, type, pageSize, pageIndex, successCallback, failCallBack) => {
    let body = {
        userId: userId,
        type: type,
        pageSize: pageSize,
    ...
    return dispatch => {
        ...
        return Util.post('/webOrder/getWebOrderList', body, (response) => {
            ...
            if (response.status == 0) {
                ...
                if (response.body.orderList !== null && response.body.orderList instanceof Array && successCallback && response.body.orderList.length > 0)
                    successCallback();
                console.log('---------->', getAry)
                if (type === 1) {
...
export let makeSureReceptPost = (orderId, userId, ary, completedAry, allAry) => {//确认收货
    let body = {
        orderId: orderId,
        userId: userId,
    }
...
//获取退货退款详情
export let getOrderDetail = (userId, orderId) => {
    let body = {
        userId: userId, orderId: orderId,
    }
    console.log('订单详情', body);
    return dispatch => {
        dispatch(showLoading(true));
