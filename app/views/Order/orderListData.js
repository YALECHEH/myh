...
class OrderListData extends Component {
    ...
    componentDidMount() {
        ...
        OrderListAction.getOrderList([], userID, 0, this.pageSize, this.pageNow, () => {
            setTimeout(() => {
    ...
    onRefresh() {
        ...
        setTimeout(() => {
            this.getMoreData = false;
            let userID = db.readUserInfo()['userId'];
            OrderListAction.getOrderList([], userID, 4, this.pageSize = 5, this.pageNow = 1, () => {
            }, () => {
            });//传相应的数组
            this.setState({
                refreshing: false,
    ...
    onClick(key, rowData) {
        ...
        switch (key) {
            ...
            case 3:
            case 4:
                OrderListAction.isCanReFunds(userID, rowData.orderId, (data) => {
                    //成功回
                    router.push({
                        pathname: contants.commonUrl + '/applyService',
                        state: {
                            returnMoney: data.returnMoney == null ? rowData.amount : (rowData.amount - parseFloat(data.returnMoney)),
    ...
    onEndReached() {
        ...
        if (this.getMoreData === false && OrderTab.isShow === false) {
            ...
            const {OrderTab, OrderListAction} = this.props;
            let userID = db.readUserInfo()['userId'];
            OrderListAction.getOrderList(OrderTab.allList, userID, 4, this.pageSize = 5, this.pageNow + 1, () => {
                this.getMoreData = false;
                    ...
                    <div className="order_list_item" onClick={() => this.onItemClick(rowData)}>
                        ...
                        <div className="order_list_item_bottom">
                            ...
                            <div className=" order_bottom_btn">
                                ...
                                {/*退款中（已发货）*/}
                                {(obj.status == 6 || obj.status == 7) && <div style={{float: 'right', display: 'flex'}}>
                                    <button name='reimburse' className="order_button_refunds" onClick={() => {
